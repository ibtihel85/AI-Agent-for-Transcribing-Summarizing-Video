def chunk_text(text: str, chunk_size: int = 2000, overlap: int = 200) -> list:
    ...
    :param chunk_size: Maximum chunk size in characters (default: 200)
    :param overlap: Number of characters to overlap between chunks (default: 20)
...
def chunked_summarize(text: str, summarize_func, max_chunk_size: int = 2000) -> str:
    """
    Summarizes a large text by splitting it into chunks, summarizing each chunk
    individually, and then combining those summaries into a final summary.

    :param text: The full text to be summarized
    :param summarize_func: A function that takes a string and returns its summary
                          (e.g., a call to an LLM summarization API or model)
    :param max_chunk_size: Approximate maximum characters per chunk (default: 2000)
    :return: A single string containing the combined summary of all chunks
    """
